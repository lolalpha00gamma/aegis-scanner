...
Anlegen mit SFace. Desktop-Test: `AegisTest.zip` auf den Schreibtisch, Prozent in der App.
...
hypot-Aspect Ghost. leftoverTrailWriteHash live. roiEvery aus dt.

## Neu in 2.1.247 alpha

leftoverPick twinPair nil ohne Live-Paar. liveRoi/skipDetect every 2. Helios 1.6.100.

## Neu in 2.1.246 alpha


leftoverPick twinPair Live (Ada+Bob kein Gallery-Veto). Helios 1.6.99.

## Neu in 2.1.244 alpha

leftoverPickSameShot Rank. leftoverAmbiguousBlocks Same-shot kein Schärfe-Ada. leftoverPickArgmax sameShot. leftoverOpenSetUnsure sameShot. Helios 1.6.97.

## Neu in 2.1.207 alpha

pairLast Value-Remint, leftoverTracks Yaw/Kalman/Blink. Helios 1.6.45.

## Neu in 2.1.206 alpha

leftoverTracksUnpack Identität, Print-Index in FaceEngine, Twin-Lock PrintCell. Helios 1.6.44.
...
Print-Yaw persist, Detect≠Print, Twin-Bin ¾L/¾R, Overlay-Lerp, Blink-Assign. Helios 1.6.40.

## Neu in 2.1.201 alpha

Trail nur Live-next. Median + Gallery-Anker. Blur-Skip ohne Stamp. Coast nur Commit. Helios 1.6.39.

## Neu in 2.1.199 alpha

leftoverPrintYaw Stamp, Merge hält committed, Print-EMA Gleichgewicht.

## Neu in 2.1.198 alpha

leftoverHoldBin |yaw|, nameHist × dt, Print-EMA, Peak-Hold verdrahtet.

## Neu in 2.1.197 alpha

leftoverAssignPrintCell (0 ≠ nil), x-Fill nur ungemessen, SameBin ohne Yaw tot, Print-Steal 0,80.

## Neu in 2.1.196 alpha

leftoverAssignPrintOk (Taufe 0,80, nicht Hold 0,64), Twin-Yaw-Cull, Print-Diversity 0,98.

## Neu in 2.1.109 alpha

JPEG-Probe vor Taufe, Hold-Bins persist Schema 5, IoU-Jump 0,40, Per-Bin Adopt, 0° Orient.

## Neu in 2.1.108 alpha

Spark-Peek im Tick, PickLuma verdrahtet, BaptizeGate, Blink-Streak, Name-AND, Score-Tick, Thermal Floor.

## Neu in 2.1.107 alpha

Hash-Spark überlebt UUID-Steal, Spark-Hold 2 Frames verdrahtet, leftoverPickLuma.

## Neu in 2.1.104 alpha

Live 15 fps, kein RotationCoordinator, leftoverTrailWriteOk ohne Yaw-Block, Spark ¾ ohne UUID-Mix.

## Neu in 2.1.103 alpha

availableVideoPixelFormatTypes, videoOut — xcodebuild 2.1.101/102 tot.

## Neu in 2.1.102 alpha

leftoverBaptizeBoth ohne roh-Fallback, ¾-Overlay ohne Frontal-Trail, leftoverNameFromHold.

## Neu in 2.1.81 alpha

Kalman-Predict auf leerem Frame, leftoverPending folgt Adopt-ID, HOLD-Chip 0,4 s.

## Neu in 2.1.80 alpha

Kalman am Ghost, Latch-Hold 4 s, leftoverPending bleibt.

## Neu in 2.1.79 alpha

Overlay-Namen am Ghost, Ghost-TTL 4 s, Profil-Yaw 0,45.

## Neu in 2.1.78 alpha

Leerer-Frame-Streak, Yaw-Floor 0,70, Same-shot Twin 0,88.

## Neu in 2.1.77 alpha

0,62-Label, WEG kein Fremder, Miss-3, 8 fps Adopt 0,6 s, TWIN-Farbe.

## Neu in 2.1.76 alpha

TWIN? hält Streak, 0,62 scharf kein Gast, WEG-Pin 0,12, WEG-Countdown.
