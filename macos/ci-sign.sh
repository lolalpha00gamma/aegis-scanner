...
import_cert() {
  ...
  if printf '%s' "$CERT_B64" | grep -q 'BEGIN '; then
    printf '%s\n' "$CERT_B64" > "$tmp"
  else
    printf '%s' "$CERT_B64" | base64 --decode > "$tmp" 2>/dev/null || {
  ...
  local imported=0
  if security import "$tmp" -k "$KC_PATH" -P "$CERT_PW" -A -t cert -f pkcs12 \
      -T /usr/bin/codesign -T /usr/bin/security >/dev/null 2>&1; then
    imported=1
  elif security import "$tmp" -k "$KC_PATH" \
      -T /usr/bin/codesign -T /usr/bin/security >/dev/null 2>&1; then
    imported=1
  fi
  rm -f "$tmp"
