...
enum MatchMathTests {
    ...
    static func main() {
        ...
        ok(!MatchMath.liveEmitAllows(busy: true), "Emit busy tot")
        ok(!MatchMath.liveEmitAllows(busy: true, busyFor: 0.40), "Emit 400 ms hält")
        ok(MatchMath.liveEmitAllows(busy: true, busyFor: 1.20), "Emit hung nach 1 s")
        near(MatchMath.liveMinIntervalFromVision(base: 1.0 / 15.0, visionMs: 0), 1.0 / 15.0, 1e-6, "Vision 0 = base")
        ok(MatchMath.liveMinIntervalFromVision(base: 1.0 / 15.0, visionMs: 150) >= 0.13, "Vision 150 ms hebt Interval")
        ok(MatchMath.liveMinIntervalFromVision(base: 0.20, visionMs: 80) >= 0.19, "schnelles Detect bleibt base")
        ...
        ok(MatchMath.leftoverTrackKind(miss: 2) == .coast, "Track coast")
        ...
        ok(MatchMath.liveEmitPendingWhileBusy(), "Pending Drop-oldest")
        ok(MatchMath.enrollBlocksWithoutBlink(liveFace: true, haveBlink: false), "Live ohne Blink tot")
        ok(!MatchMath.enrollBlocksWithoutBlink(liveFace: false, haveBlink: false), "Foto ohne Blink")
        ok(!MatchMath.enrollBlocksWithoutBlink(liveFace: true, haveBlink: true), "Live mit Blink")
        ok(MatchMath.leftoverCoastAgeChip(kind: .coast, coastAt: 10, now: 10.30)?.contains("coast") == true, "Coast Age")
        ok(MatchMath.leftoverCoastAgeChip(kind: .live, coastAt: 10, now: 10.30) == nil, "Coast Age live tot")
        ok(MatchMath.cameraMutexWatchdogAction(killEnabled: false) == "pause", "Watchdog Pause")
        ok(MatchMath.cameraMutexWatchdogShouldPause(action: "pause", holder: "helios", owner: "aegis", wrote: false), "Pause Partner")
        ...
        ok(MatchMath.holdStillSkipIoU(dt: 0.125) < MatchMath.holdStillSkipIoU(dt: 0.04) - 0.10, "8 fps IoU-Floor tiefer")
        near(MatchMath.holdStillSkipIoU(dt: 0.04), 0.70, 0.01, "24 fps IoU-Floor 0,70")
        near(MatchMath.holdStillSkipIoU(dt: 0.125), 0.50, 0.01, "8 fps IoU-Floor 0,50")
        ok(MatchMath.holdStillNeedOf(dt: 0.125) > MatchMath.holdStillNeed, "8 fps Hold-Still länger")
        ok(MatchMath.holdStillNeedOf(dt: 0.016) <= MatchMath.holdStillNeed + 0.01, "24 fps Hold-Still 0,80")
        ok(MatchMath.holdStillSkip(iou: 0.55, dt: 0.125) == false, "8 fps IoU 0,55 still")
        ok(MatchMath.holdStillSkip(iou: 0.55, dt: 0.04) == true, "24 fps IoU 0,55 skip")
        ...
        ok(MatchMath.captureLockFrameRate(30, continuity: false, external: true) == 24, "Osmo 24 statt 30")
        ...
        ok(!MatchMath.overlayTrackBeats(live: false), "Track Beat idle tot")
        ...
        ok(!MatchMath.falseAcceptJSONLHits(cosine: 0.91, floor: 0.80, decided: "Ben", expected: ""), "FA ohne Lock tot")
        ok(MatchMath.falseAcceptJSONLShouldLog(prevDecided: nil, decided: "Ada"), "FA erstes Assignment")
        ok(!MatchMath.falseAcceptJSONLShouldLog(prevDecided: "Ada", decided: "Ada"), "FA gleiches skip")
        ok(MatchMath.falseAcceptJSONLShouldLog(prevDecided: "Ada", decided: "Ben"), "FA Wechsel log")
        near(MatchMath.falseAcceptJSONLCosine(hold: 0.91, emaPercent: 80), 0.91, 0.001, "FA hold Cosine")
        near(MatchMath.falseAcceptJSONLCosine(hold: nil, emaPercent: 91), 0.91, 0.001, "FA ema Prozent")
        ...
        ok(MatchMath.overlayTrackLost(confidence: 0.05), "Track lost")
        ok(!MatchMath.overlayTrackLost(confidence: 0.40), "Track hält")
        ok(MatchMath.overlayTrackForcesDetect(lost: 1, live: 2), "Track lost Detect")
        ok(!MatchMath.overlayTrackForcesDetect(lost: 0, live: 2), "Track ok kein Detect")
        ok(!MatchMath.overlayTrackForcesDetect(lost: 1, live: 0), "Track lost ohne Live tot")
        ok(MatchMath.peopleAlbumScanCap() == 12, "People scan 12")
        let yawPick = MatchMath.peopleAlbumYawPick(bins: [0, 0, -1, 1], cap: 3)
        ok(yawPick == [0, 2, 3], "People yaw Front+L+R")
        ok(MatchMath.peopleAlbumYawDiverse(bins: [0, -1, 1]), "People yaw diverse")
        ok(!MatchMath.peopleAlbumYawDiverse(bins: [0, 0, 0]), "People yaw nur Front tot")
        ok(MatchMath.peopleAlbumFetchAny(limited: true), "People limited any")
        ok(!MatchMath.peopleAlbumFetchAny(limited: false), "People full Faces")
        ok(MatchMath.peopleAlbumLimitedStatus(seeded: 0, limited: true).contains("eingeschränkt"), "People limited leer")
        ok(MatchMath.peopleAlbumLimitedStatus(seeded: 2, limited: true).contains("eingeschränkt"), "People limited seeded")
        let matrix = MatchMath.falseAcceptPairMatrix(heat)
        ok(matrix.contains("Ada→Ben×2") && matrix.contains("Ada→Cara×1"), "FA matrix")
        ok(MatchMath.falseAcceptPairMatrix([]) == "FA —", "FA matrix leer")
        ok(MatchMath.nameTemporalNeed(family: false, dt: 0.125) == 5, "Temporal 8 fps Floor 5")
        ok(MatchMath.nameTemporalNeed(family: false, dt: 0.04) >= 5, "Temporal 24 fps ≥ 5")
        ok(MatchMath.nameTemporalVote(["Ada", "Ada", "Ada"], dt: 0.125) == nil, "3 Ticks taufen nicht")
        ok(MatchMath.nameTemporalVote(["Ada", "Ada", "Ada", "Ada", "Ada"], dt: 0.125) == "Ada", "5 Ticks taufen")
        ok(MatchMath.nameTemporalVote(["Ada", "Ben", "Ada", "Ada", "Ada"], dt: 0.125) == nil, "Mix unter 5 tot")
        ok(MatchMath.printCaptureQualitySkip(0.20), "Capture 0,20 skip")
        ok(!MatchMath.printCaptureQualitySkip(0.50), "Capture 0,50 Print")
        ok(MatchMath.printCaptureQualitySkip(0.34), "Capture 0,34 skip Floor")
        ok(!MatchMath.printCaptureQualitySkip(0.35), "Capture 0,35 hält")
        ok(MatchMath.peopleAlbumDuplicate(cosine: 0.91), "People dup 0,91")
        ok(!MatchMath.peopleAlbumDuplicate(cosine: 0.80), "People 0,80 neu")
        ok(MatchMath.peopleAlbumDuplicate(cosine: 0.89), "People dup Floor 0,89")
        ok(MatchMath.enrollQualityMeter(capture: 0.20, sharpness: 0.10, yawCoverage: 0) .contains("schwach"), "Q schwach")
        ok(MatchMath.enrollQualityMeter(capture: 0.90, sharpness: 0.85, yawCoverage: 1).contains("gut"), "Q gut")
        ok(MatchMath.enrollQualityMeter(capture: 0.55, sharpness: 0.50, yawCoverage: 0.40).hasPrefix("Q "), "Q mittel")
        ok(MatchMath.lookalikeNegative(best: 0.91, second: 0.88), "Lookalike 0,91/0,88")
        ok(!MatchMath.lookalikeNegative(best: 0.91, second: 0.70), "Lookalike Gap groß tot")
        ok(!MatchMath.lookalikeNegative(best: 0.75, second: 0.74), "Lookalike beide niedrig tot")
        ok(MatchMath.lookalikeNegativeScores([0.90, 0.87, 0.40]), "Lookalike scores")
        ok(!MatchMath.lookalikeNegativeScores([0.90]), "Lookalike eine tot")
        ...
        ok(MatchMath.peopleAlbumSkipHidden(title: "Ada", subtypeRaw: 205), "subtype hidden")
        ok(!MatchMath.peopleAlbumSkipHidden(title: "Ada"), "Ada seeden")
        ok(MatchMath.guestTTLNeed() == 30, "Guest TTL 30 s")
        ok(MatchMath.cosineTickMedian([0.91, 0.70, 0.72]).map { abs($0 - 0.72) < 0.001 } == true, "Median 0,91/0,70/0,72 = 0,72")
        ok(MatchMath.cosineTickMedian([0.91, 0.91, 0.91]).map { abs($0 - 0.91) < 0.001 } == true, "Median genuine")
        ok(MatchMath.cosineTickMedian([] ) == nil, "Median leer tot")
        ...
        ok(!MatchMath.enrollSMReadyFromChip("ENROLL ●●●●", needProfile: true), "Twins P Pflicht")
        ok(MatchMath.enrollSMReadyFromChip("ENROLL ●●●●●", needProfile: true), "Twins P fertig")
        ok(!MatchMath.enrollSMReadyFromChip("ENROLL P", needProfile: true), "Twins P-Chip tot")
        ...
        ok(MatchMath.peopleAlbumStillDup(yawA: 0.02, yawB: 0.01, captureA: 0.50, captureB: 0.51), "People still same pose")
        ok(!MatchMath.peopleAlbumStillDup(yawA: 0.02, yawB: 0.40, captureA: 0.50, captureB: 0.51), "People L≠Front")
        ok(MatchMath.displayNameSuffix(base: "Ada", taken: []) == "Ada", "Ada frei")
        ok(MatchMath.displayNameSuffix(base: "Ada", taken: ["Ada"]) == "Ada 2", "Ada 2")
        ok(MatchMath.displayNameSuffix(base: "Ada", taken: ["Ada", "Ada 2"]) == "Ada 3", "Ada 3")
        ok(MatchMath.galleryCompactDrops(capture: 0.20, remaining: 2), "Compact drop")
        ok(!MatchMath.galleryCompactDrops(capture: 0.20, remaining: 1), "Compact last still hält")
        ok(!MatchMath.galleryCompactDrops(capture: 0.50, remaining: 3), "Compact sharp hält")
        let b1 = MatchMath.burstRejectTick(hash: "6.6.4.6", prevHash: nil, streak: 0, dtMs: 50)
        ok(b1.streak == 1 && !b1.reject, "Burst start")
        let b2 = MatchMath.burstRejectTick(hash: "6.6.4.6", prevHash: "6.6.4.6", streak: 1, dtMs: 40)
        ok(b2.streak == 2 && !b2.reject, "Burst 2")
        let b3 = MatchMath.burstRejectTick(hash: "6.6.4.6", prevHash: "6.6.4.6", streak: 2, dtMs: 40)
        ok(b3.streak == 3 && b3.reject, "Burst 3 skip")
        let bGap = MatchMath.burstRejectTick(hash: "6.6.4.6", prevHash: "6.6.4.6", streak: 2, dtMs: 400)
        ok(bGap.streak == 1 && !bGap.reject, "Burst gap reset")
        ok(MatchMath.captureLocksAE(continuity: true), "Continuity AE-Lock")
        ok(!MatchMath.captureLocksAE(continuity: false), "Mac kein AE-Lock")
        ...
        ok(MatchMath.cameraMutexPtsWall(now: 1_700_000_000, mediaPts: 12.4) == 1_700_000_000, "PTS Wall vor Media")
        near(MatchMath.cameraMutexPtsWall(now: 12.4, mediaPts: 1_700_000_000), 1_700_000_000, 1, "PTS Wall Media unix")
        let palmLine = MatchMath.cameraMutexLine(owner: "helios", pid: 42, now: 100, gen: 3, pts: 1.5, palm: (0.40, 0.55, 0.12))
        near(Double(MatchMath.cameraMutexPalm(palmLine)?.x ?? -1), 0.40, 1e-6, "Mutex Palm X")
        near(Double(MatchMath.cameraMutexPalm(palmLine)?.w ?? -1), 0.12, 1e-6, "Mutex Palm W")
        ok(MatchMath.cameraMutexParse(palmLine, now: 101) == "helios", "Mutex Palm Parse")
        ok(MatchMath.cameraMutexProtocol(palmLine) == 2, "Mutex Palm v2")
        ok(MatchMath.cameraMutexPalm(ptsLine) == nil, "Mutex ohne Palm tot")
        ok(MatchMath.cameraMutexPalmSkip(holder: "helios", continuity: true), "Palm skip Continuity")
        ok(!MatchMath.cameraMutexPalmSkip(holder: "helios", continuity: false), "Palm Built-in tot")
        ok(!MatchMath.cameraMutexPalmSkip(holder: "aegis", continuity: true), "Palm eigen tot")
        ok(!MatchMath.cameraMutexPalmSkip(holder: nil, continuity: true), "Palm frei tot")
        let face = FaceBox(x: 0.30, y: 0.45, width: 0.20, height: 0.25)
        let palmBox = MatchMath.cameraMutexPalmBox((0.40, 0.55, 0.12), imageW: 1, imageH: 1)
        ok(MatchMath.leftoverPrintSkipHits(face: face, skipBoxes: [], palm: palmBox), "Palm occludes")
        ok(!MatchMath.leftoverPrintSkipHits(face: face, skipBoxes: []), "ohne Palm tot")
        let farPalm = MatchMath.cameraMutexPalmBox((0.90, 0.90, 0.08), imageW: 1, imageH: 1)
        ok(!MatchMath.leftoverPrintSkipHits(face: face, skipBoxes: [], palm: farPalm), "Palm weit tot")
        ok(MatchMath.cameraMutexPalmHits(face: face, palm: palmBox), "PalmHits")
        near(MatchMath.cameraMutexPalmIoU(), 0.18, 1e-9, "Palm IoU 0,18")
        ...
        ok(soloStale[soloStaleNew] == 0.77, "Solo vor x-Match trotz stale LastHash")
