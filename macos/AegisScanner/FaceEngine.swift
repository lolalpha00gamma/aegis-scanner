...
enum FaceEngine {
    ...
    static func detect(in image: CGImage, mediaId: UUID, tiles: Bool = true, orientation: CGImagePropertyOrientation = .up, minSharpness: Double = MatchMath.sharpnessFloor, continuity: Bool = false, cheapGraph: Bool = false, live: Bool = false, skipPrints: Bool = false, roi: FaceBox? = nil, skipPrintBoxes: [FaceBox] = [], skipPrintPalm: FaceBox? = nil, skipPrintPalms: [FaceBox] = []) throws -> [FaceObservation] {
        ...
        var out = try detectOnce(in: work, mediaId: mediaId, originX: cropOrigin.x, originY: cropOrigin.y, imageWidth: w, imageHeight: h, orientation: orientation, cheapGraph: cheapGraph, continuity: continuity)
        ...
        return stampPrints(boxed, from: image, orientation: orientation, continuity: continuity, minSharpness: minSharpness, skipPrintBoxes: skipPrintBoxes, skipPrintPalm: skipPrintPalm, skipPrintPalms: skipPrintPalms)
    }
    ...
    private static func detectOnce(
        ...
        for face in observations where face.confidence >= minConfidence {
            ...
            let sharpness = MatchMath.leftoverSharpnessOf(sharpnessScore(faceCrop), videoRange: continuity) ?? 0
    ...
    static func printWeights(_ faces: [FaceObservation]) -> [(id: UUID, weight: Double, slot: String)] {
        faces.map { f in
            ...
            let w = has ? MatchMath.centroidWeight(
                capture: f.quality.capture,
                sharpness: f.quality.sharpness,
                frontal: f.quality.frontal,
                yawAbs: f.quality.yaw
    ...
    private static func stampPrints(_ faces: [FaceObservation], from image: CGImage, orientation: CGImagePropertyOrientation = .up, continuity: Bool = false, minSharpness: Double = MatchMath.sharpnessFloor, skipPrintBoxes: [FaceBox] = [], skipPrintPalm: FaceBox? = nil, skipPrintPalms: [FaceBox] = []) -> [FaceObservation] {
