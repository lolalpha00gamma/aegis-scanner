...
2.1.246 leftoverPick twinPair Live, Fallback Gallery. Ohne Vec Hard-Veto. ROI Full/8 + skipDetect/4 = Walk-in 1 s.

1. **leftoverPick twinPair nil** ohne Live-Paar.
2. **liveRoiPeriodicFull every: 2.** Default 8 bleibt.
3. **leftoverDetectSkipTick every: 2.** Default 8 bleibt.
4. Tests + VERSION = Models = MARKETING 2.1.247 (Build 272). Schema 15 bleibt.

leftoverHoldsTrack yawAbs: nil bewusst. leftoverScore Twin-Penalty Rank unverändert. leftoverPrintYawMerge printCommitted sitzt (2.1.245). leftoverPick Live-Twin sitzt (2.1.246).

## Erweiterung (neu, 2.1.247)

1. **CameraBroker XPC + IOSurface** mit Helios. P0. Spec `docs/CameraBroker.md`.
...
2.1.206 Unpack/Move/Print-Index/Twin-Lock. leftoverPairLast nur faceMaps. AssignAtomic clobberte pairLast mit Hold-Key. Yaw/Kalman/Blink 20 Drops.

1. **leftoverAssignAtomicAll** remintet pairLast-Value. Hold bleibt stored-Key.
2. **leftoverPairLastPick** Unpack gewinnt, leer → faceMaps.
3. **leftoverTracksPack** Yaw / Kalman-Vx/Vy / Blink. Remint überlebt.
4. Tests + VERSION = Models = MARKETING 2.1.207 (Build 232). Schema 15 bleibt.

## Erweiterung (neu, 2.1.207)
...
10. **Gemeinsamer CVPixelBuffer** Detect+Print, JPEG nur Export.
11. **VNTrackObjectRequest** Box zwischen Detect-Ticks.
12. **Hung-Detect Cancel** nach 1 s.
13. **LiveCapture nicht @MainActor.**
14. **420f vs 420v** Color-Space Continuity/Built-in.
15. **Eine Fixture Restart+Twin+AssignLive** statt 40 Bool-Orakel.
16. **Overlay identity-Lerp unabhängig von Assign.**
17. **Blink-Liveness auf Assign.**
18. **Match-Log JSONL** für False-Accept Replay.
19. **Drop-in `.mlmodel`** Print-Backbone ohne MatchMath-Rewrite.
20. **Print-Cache leftoverLastHash × Yaw-Bin** — gleicher Hash+Bin kein zweiter VNFacePrint.
...
Neu in 2.1.173: FaceTrack-PredictHeld, Coast gallery, Lookaway-Tried tot.
...
1. **`leftoverCaptureHistOf`.**
2. **`leftoverCaptureChip` + leftoverHoldLabel yawAbs.**
3. **`leftoverScoreHeat` / `leftoverScoreSoftmax`.**
