...
2.1.214 VNTrack one-shot, People nur Name, FA ohne Paar. Track-Handler jedes Frame neu. Volle Bins nochmal Print.

- **VNTrack persist.** Sequence+Observation über skipDetect. Detect reseedet.
- **Enroll-SM skippt volle Bins.**
- **People 3 Stills.** Name + Detect-Referenzen. limited-auth.
- **FA Pair-Heatmap.** Ada→Ben×n.
- Tests + VERSION = Models = MARKETING 2.1.215 (Build 240). Schema 15 bleibt.
...
2.1.200: Hold-Still × fps, Overlay-Taufe, Print-EMA × dt. Trail append prev jedes Tick. Coast-Print trotz Diversity-Skip. Blur-Skip stempelt Yaw. Mean ab 2 Frames bei 8 fps = 50 % Glück.

- **leftoverPrintTrailNext** nur Live-next. Gallery-Print ist Anker, nicht Sample.
- **leftoverPrintBlend** Median + Gallery-Anker. Mean nur wenn Last ≈ Median (Cosine ≥ 0,97).
- **leftoverPrintCommitOk.** Blur-Skip schreibt keinen Trail und stempelt kein Yaw.
- **leftoverCoastPrintMerge commitIds.** Coast nur nach Gallery-Commit.
- Helios **1.6.39:** Approach+Reach, Finger-Kontakt, q-Gate, Tip-Konfidenz.
- Tests + VERSION = Models = MARKETING 2.1.201 (Build 226). Schema 15 bleibt.
...
2.1.194: Twin-Overlay Detect-Fallback. Unmatched Remint unmountet die zweite Box. leftoverBlinkSeen nur Detect-UUID. Box-Hash als Blink wäre Enroll-Leak.

- **leftoverBlinkSeenOf** — Detect + Identity. Hash nie für Liveness. Coach nur owner.
- **leftoverOverlayBoxHash** — Twin und unmatched ForEach überlebt Remint.
- **liveFrameTapSkipsCGImage** — pending ohne Conversion < 80 ms. DidWake liveRoiSkipOnce.
- Helios **1.5.196:** Sleep-Fill tot, DisplayLink 300 ms stumm, PINCH an der Hand.
- Tests + VERSION = Models = MARKETING 2.1.195 (Build 220). Schema 15 bleibt.

## Neu in 2.1.194 alpha

2.1.193: Overlay identityId Dedup droppte den Twin. Helios Silence-Age CACurrent nach Sleep 0. meanConfidence inkl. Tips.

- **leftoverOverlayRowId** — identityId schon da → Detect-ID, Twin-Box bleibt.
- Helios **1.5.195:** frameSilenceAge Wall, meanConfidence ohne Tips, Pause-Resume nur wenn gewollt.
- Tests + VERSION = Models = MARKETING 2.1.194 (Build 219). Schema 15 bleibt.
...
2.1.186 hop't FrameTap auf der Capture-Queue, aber jedes CGImage spawnt weiter `Task { @MainActor }`. Detect 80–200 ms, Continuity 8–15 fps → Task-Stau, Jank, Namen hinterher.

- **liveEmitAllows** — FrameTap `emitBusy` vor CGImage. markFrameConsumed erst nach Detect. Hung 1 s.
- **liveMinIntervalFromVision** — visMs hebt das Interval (150 ms Detect → kein 15 fps).
- Helios **1.6.33:** Pinch-Klick, Zwei-Pinzetten, Tastatur-Dwell × dt.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.187 (Build 212). Schema 15 bleibt.

## Neu in 2.1.186 alpha

PTS-Wall blieb intern: Mutex ohne gemeinsame Uhr. FrameTap hop't jedes CGImage auf Main. skipPrint ignorierte Yaw — Profil verdünnt die Bank.

- **cameraMutexLine 5. Feld** Sample-PTS. obsFillUsesMutexPts Skew 80 ms.
- **liveFrameTapEmitsOnCaptureQueue** — FrameTap ohne Main-Hop. LibraryStore `Task { @MainActor }`.
- **printQuality** sharpness×(1-|yaw|/90°). skipPrint yaw.
- Helios **1.5.187:** Mutex-PTS, Pinch 18/28, pinchReleaseFiresClick.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.186 (Build 211). Schema 15 bleibt.

## Neu in 2.1.185 alpha

IoU-Floor nur fps: nah zu weich, weit zu hart. FrameTap stampte Date() obwohl PTS schon da war. Until nach IoU-Adopt leer → Restart wischt Ada.

- **leftoverOverlayPeakIoUFloorArea** — Box-Fläche auf fps-Floor. Peak und NameLock.
- **ptsWallStamp** FrameTap Interval+Emit.
- **leftoverNameLockUntilFillHeld** nach Coast — gallery.json Remaining.
- Helios **1.5.186:** Overlay-Lerp 8 fps, Ghost-Knochen, PTS-Wall, Pinch-Drop.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.185 (Build 210). Schema 15 bleibt.
...
Overlay-Peak saß, leftoverMirrorPending remintete PeakHeld nicht. Assign-Live → Overlay „?“ Tick 1. Helios Canvas fraß Ghost-Knochen.

- **leftoverAssignAtomic PeakHeld/Remain** in leftoverMirrorPending.
- Helios **1.5.181:** overlayDrawsGhost, Kalman-P über Fill-Gap.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.182 (Build 207). Schema 15 bleibt.

## Neu in 2.1.181 alpha

Remint-UUID 1–3 Frames Overlay „?“ trotz Ada. leftoverOverlayGuestOf Need 3, Hist keep 1. Helios Hub-Doppelframe snappt Lerp, Floor 0,05 freeze, Reanchor hart 8 px.

- **leftoverOverlayPeakGuest / PeakName / PeakAdvance.** Ada hält 3 Frames über „?“. Display dekrementiert nicht.
- leftoverOverlayPeakHeld/Remain remint-drop, unique live Set.
- Helios **1.5.180:** Lerp-Hitch Cap 2, overlayLerpDtOf, pointerReanchorRms.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.181 (Build 206). Schema 15 bleibt.
...
leftoverTried nicht auf Unsure. 3× `?` leftoverClearStreak. Coast-Print TTL 2 s. Name-Hist nach Remint keep 1. Helios 1.5.169 Hist-Prior vor Conf.

- Tests + VERSION = Models = MARKETING_VERSION 2.1.171 (Build 196). Schema 15 bleibt.
...
PairCommit nach Remint Majority neu. Kalman Keep nur missCoast — Ghost freeze. KeepBoxes droppt Ghost-Kalman. IoU-Reset hart 0,40.

- **leftoverPairCommitKeeps.** Majority hält committed==proposed.
- **leftoverHoldKalmanPredictOnly.** Restore / Miss / Ghost-only Predict.
- **leftoverKeepBoxes Ghost-Kalman.** leftoverHoldKalmanJumpPref 0,30–0,50.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.148 (Build 173).

## Neu in 2.1.147 alpha

Restore las Remaining aus der Live-Galerie. HashTrail TTL startet nach Restore. KeepBoxes vor Survive. Hungarian Pad 0,40 explodiert.

- **loadBackupPayload remaining.** leftoverHashTrail remaining Schema 15.
- **leftoverKeepBoxes nach Survive.** leftoverAssignHungarianWide → FillX.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.147 (Build 172).
...
Miss-Coast hart 1 Tick Indoor tot. JPEG Schema-11 at=now. Kalman IoU-Reset Tick 1. Ranked Occupied blockt Exact. Hungarian n=5.

- **leftoverHoldMissNeedPref 1–3 Default 2.** leftoverHoldMissNeedAuto dt. Slider.
- **leftoverJpegRestoreAt.** Schema 11 stale. leftoverHoldKalmanSkipReset 2 Ticks.
- **leftoverOccupiedRankBlocks / leftoverOccupiedMergeYaw.** leftoverHoldKalmanVelDecay. HungarianX n≤6.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.145 (Build 170).
...
Detect-Drop wischt persist leftoverHold. Occupied live Rank. JPEG Encode ungekürzt. Kalman RAM-only. Hungarian n=4.

- **leftoverHoldMissCoast 1 Tick.** Survive / LastHash / Tick halten. leftoverPredictOnMissCoast.
- **leftoverOccupiedMerge Spatial-emit.** leftoverJpegByHashCapped Encode 64.
- **Schema 12 leftoverHoldKalman persist.** leftoverAssignHungarianX n≤5.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.143 (Build 168).
...
CI rot seit 2.1.139: leftoverHoldXMatch `occupied:` vor `pad:` — swiftc bricht Remint.

- **leftoverHoldXMatch** pad vor occupied, 4 Call-Sites.
- **leftoverAssignHungarianX** `let used`.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.141 (Build 166).
...
Tick-Copy rettete Hash, nicht PairLast/Streak/Commit. x-Pad 0,12 ohne Hash tot sobald der Kopf 18 cm ging.

- **`leftoverFillXRescue` 0,28.** Dritter Remint-Pass. Far 0,90 bleibt tot.
- **`leftoverHoldMove` / `leftoverHoldMoveId`.** Mirror Pending bewegt Pair* Keys + Commit-Value.
- Tests + VERSION = Models = MARKETING_VERSION 2.1.131 (Build 157).
